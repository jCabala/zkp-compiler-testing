...
def solve_r1cs(circom_path: Path):
    """
...
def export_r1cs(circom_path: Path, output: Path):
    """
...
@cli.command(name="generate-proof")
@click.argument('circom_path', type=click.Path(exists=True, path_type=Path))
...
def generate_proof(circom_path: Path, input_json: Path, ptau: Path, outdir: Path | None):
    """
