...
def _assert_solve(smt_path: Path, solver: str, expected: str, *, with_hints: bool):
	args = [
		"solve",
		str(smt_path),
		"--zk-dsl",
...
def test_zokrates_solves_tiny_ff_unsat_formula(tmp_path: Path):
	smt_path = tmp_path / "tiny_ff_unsat.smt2"
	smt_path.write_text(TINY_FF_UNSAT)
	_assert_solve(smt_path, "cvc5", "unsat", with_hints=False)
