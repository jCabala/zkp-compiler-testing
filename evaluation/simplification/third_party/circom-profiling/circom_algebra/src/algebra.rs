...
// === PROFILING: global operator counters ===
static OP_ADD: AtomicUsize = AtomicUsize::new(0);
static OP_SUB: AtomicUsize = AtomicUsize::new(0);
static OP_MUL: AtomicUsize = AtomicUsize::new(0);
static OP_DIV: AtomicUsize = AtomicUsize::new(0);
static OP_IDIV: AtomicUsize = AtomicUsize::new(0);
static OP_MOD: AtomicUsize = AtomicUsize::new(0);
static OP_POW: AtomicUsize = AtomicUsize::new(0);
static OP_SHIFT_L: AtomicUsize = AtomicUsize::new(0);
static OP_SHIFT_R: AtomicUsize = AtomicUsize::new(0);
static OP_BIT_OR: AtomicUsize = AtomicUsize::new(0);
static OP_BIT_AND: AtomicUsize = AtomicUsize::new(0);
static OP_BIT_XOR: AtomicUsize = AtomicUsize::new(0);
static OP_NON_QUADRATIC: AtomicUsize = AtomicUsize::new(0);

pub fn print_operator_profiling() {
    eprintln!("{{\"event\":\"operator_counts\",\"add\":{},\"sub\":{},\"mul\":{},\"div\":{},\"idiv\":{},\"mod\":{},\"pow\":{},\"shift_l\":{},\"shift_r\":{},\"bit_or\":{},\"bit_and\":{},\"bit_xor\":{},\"non_quadratic_results\":{}}}",
        OP_ADD.load(Ordering::Relaxed),
        OP_SUB.load(Ordering::Relaxed),
        OP_MUL.load(Ordering::Relaxed),
        OP_DIV.load(Ordering::Relaxed),
        OP_IDIV.load(Ordering::Relaxed),
        OP_MOD.load(Ordering::Relaxed),
        OP_POW.load(Ordering::Relaxed),
        OP_SHIFT_L.load(Ordering::Relaxed),
        OP_SHIFT_R.load(Ordering::Relaxed),
        OP_BIT_OR.load(Ordering::Relaxed),
        OP_BIT_AND.load(Ordering::Relaxed),
        OP_BIT_XOR.load(Ordering::Relaxed),
        OP_NON_QUADRATIC.load(Ordering::Relaxed),
    );
}
...
impl<C: Default + Clone + Display + Hash + Eq> ArithmeticExpression<C> {
    ...
    pub fn add(
        ...
        OP_ADD.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        match (left, right) {
            (NonQuadratic, _) | (_, NonQuadratic) | (Quadratic { .. }, Quadratic { .. }) => {
                OP_NON_QUADRATIC.fetch_add(1, Ordering::Relaxed);
                NonQuadratic
            }
    ...
    pub fn mul(
        ...
        OP_MUL.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        match (left, right) {
            (NonQuadratic, _)
            | (_, NonQuadratic)
            ...
            | (Linear { .. }, Quadratic { .. })
            | (Quadratic { .. }, Signal { .. })
            | (Signal { .. }, Quadratic { .. }) => { OP_NON_QUADRATIC.fetch_add(1, Ordering::Relaxed); NonQuadratic }
    ...
    pub fn sub(
        ...
        OP_SUB.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
    ...
    pub fn div(
        ...
        OP_DIV.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        match (left, right) {
            (Number { value: value_0 }, Number { value: value_1 }) => {
    ...
    pub fn idiv(
        ...
        OP_IDIV.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        match (left, right) {
            (Number { value: value_0 }, Number { value: value_1 }) => {
    ...
    pub fn mod_op(
        ...
        OP_MOD.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
    ...
    pub fn pow(
        ...
        OP_POW.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        match (left, right) {
            (Number { value: value_0 }, Number { value: value_1 }) => {
    ...
    pub fn shift_l(
        ...
        OP_SHIFT_L.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        if let (Number { value: value_0 }, Number { value: value_1 }) = (left, right) {
    ...
    pub fn shift_r(
        ...
        OP_SHIFT_R.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        if let (Number { value: value_0 }, Number { value: value_1 }) = (left, right) {
    ...
    pub fn bit_or(
        ...
        OP_BIT_OR.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        if let (Number { value: value_0 }, Number { value: value_1 }) = (left, right) {
    ...
    pub fn bit_and(
        ...
        OP_BIT_AND.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        if let (Number { value: value_0 }, Number { value: value_1 }) = (left, right) {
    ...
    pub fn bit_xor(
        ...
        OP_BIT_XOR.fetch_add(1, Ordering::Relaxed);
        use ArithmeticExpression::*;
        if let (Number { value: value_0 }, Number { value: value_1 }) = (left, right) {
