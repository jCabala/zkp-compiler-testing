    ...
    T: AsRef<HashSet<usize>>,
{
    ...
    let min = 350;
    let max = 1000000;
    let cluster_size = config.constraints.len();
    let num_signals = config.num_signals;
    let apply_less_ocurrences =
        cluster_size >= min &&
        cluster_size < max &&
        !config.use_old_heuristics;

    let process_name = if apply_less_ocurrences { "process_4" } else { "process_3" };

    // === PROFILING: cluster and process selection ===
    eprintln!("{{\"event\":\"cluster_simplification\",\"cluster_size\":{},\"num_signals\":{},\"process\":\"{}\",\"use_old_heuristics\":{}}}",
        cluster_size, num_signals, process_name, config.use_old_heuristics);
    ...
    for (_, v) in non_overlapping {
        LinkedList::push_back(&mut substitutions, v);
    }

    // === PROFILING: cluster result ===
    eprintln!("{{\"event\":\"cluster_result\",\"process\":\"{}\",\"cluster_size\":{},\"substitutions_produced\":{},\"remaining_constraints\":{}}}",
        process_name, cluster_size, substitutions.len(), constraints.len());
