#!/usr/bin/env bash
#
# Run N parallel circuzz instances with the Picus oracle.
#
# Usage:
#   ./run-circuzz-picus.sh
#
# Each instance runs for T_MINUTES (default 5) with a unique seed.
# Results land in obj/run-<timestamp>-circuzz-picus/instance-{0..N-1}/
#
# Override examples:
#   N_INSTANCES=3 T_MINUTES=10 ./run-circuzz-picus.sh
...
CIRCUZZ_CONFIG="fyp_experiments/artificial-bugs/configs/picus.json"

IMAGE_CIRCUZZ="${IMAGE_CIRCUZZ:-picus-with-circom}"
...
START=$(date +%s)
OBJ_DIR="$SCRIPT_DIR/obj/run-${START}-circuzz-${ORACLE}"
mkdir -p "$OBJ_DIR"
echo "circuzz-${ORACLE}" > "$OBJ_DIR/tool.txt"

echo "[preflight] oracle:    $ORACLE"
...
for (( i=0; i<N_INSTANCES; i++ )); do
    ...
    container_name="artbugs-circuzz-${ORACLE}-${i}-$$"
