#!/usr/bin/env bash
#
# Run the weak-SAT connectivity experiment for both Circom and Gnark backends
# (concurrently), then produce a connectivity distribution report.
#
# For each circuit where a satisfying witness is found, the experiment records
# how many assertions are connected (depend transitively on an input signal)
# vs. disconnected.
#
# Usage: ./run.sh [--build]
...
CIRCOM_OBJ="$CIRCUZZ_DIR/fyp_experiments/weak-sat/obj/$OBJ_DIR_SUFFIX/circom"
GNARK_OBJ="$CIRCUZZ_DIR/fyp_experiments/weak-sat/obj/$OBJ_DIR_SUFFIX/gnark"

CIRCOM_REPORT_HOST="$CIRCOM_OBJ/explore/report"
GNARK_REPORT_HOST="$GNARK_OBJ/explore/report"
CIRCOM_LOG="$CIRCOM_OBJ/explore/logs/explore.log"
GNARK_LOG="$GNARK_OBJ/explore/logs/explore.log"

mkdir -p "$CIRCOM_REPORT_HOST" "$(dirname "$CIRCOM_LOG")"
mkdir -p "$GNARK_REPORT_HOST" "$(dirname "$GNARK_LOG")"

TMP_DIR="/tmp/circuzz-weak-sat-$START"
mkdir -p "$TMP_DIR"

# Paths inside the container (source mounted at /app)
CIRCOM_REPORT_CONTAINER="/app/fyp_experiments/weak-sat/obj/$OBJ_DIR_SUFFIX/circom/explore/report"
GNARK_REPORT_CONTAINER="/app/fyp_experiments/weak-sat/obj/$OBJ_DIR_SUFFIX/gnark/explore/report"

CIRCOM_CONFIG="fyp_experiments/weak-sat/configs/circom.json"
GNARK_CONFIG="fyp_experiments/weak-sat/configs/gnark.json"
...
{
    echo "Weak-SAT Connectivity Report"
    echo "Generated: $(date)"
    echo "Seed: $SEED"
    echo "Duration: $TOOL_TIMEOUT"
    echo ""
    echo "Columns: SAT circuits | Total assertions | Connected | Disconnected | % Disconnected"
    echo ""
